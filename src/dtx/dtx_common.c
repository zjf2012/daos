...
dtx_handle_init(struct dtx_id *dti, daos_handle_t coh,
		...
		int dti_cos_count, struct dtx_memberships *mbs,
...
out:
	...
	D_DEBUG(DB_IO, "Sub init DTX "DF_DTI" for %s object "DF_UOID
...
init:
	...
	D_DEBUG(DB_IO, "Start DTX "DF_DTI" ver %u, dti_cos_count %d\n",
	  ...
	  struct dtx_memberships *mbs, struct dtx_handle *dth)
{
	...
	dtx_handle_init(dti, cont->sc_hdl, epoch, pm_ver,
			leader_oid, dti_cos, dti_cos_cnt, mbs, false,
			false, dth);

	D_DEBUG(DB_IO, "Start the DTX "DF_DTI" ver %u, dti_cos_count %d\n",
		DP_DTI(dti), dth->dth_ver, dti_cos_cnt);
