...
static void
move_sgl_cursors(daos_sg_list_t *sgl, size_t size, unsigned int *sg_idx,
		 size_t *sg_off)
{
	size_t buf_len;
...
static int
allocate_parity(struct obj_ec_parity *par, unsigned int len, unsigned int p,
		unsigned int prior_cnt)
{
	...
	for (i = prior_cnt; i < p+prior_cnt; i++) {
		...
		int recx_idx, unsigned int *sg_idx, size_t *sg_off)
{
	...
	if (s_cur != recx_start_offset)
		move_sgl_cursors(sgl, recx_start_offset % (len * k), sg_idx, sg_off);
	for ( ; s_cur + len*k <= recx_end_offset;
		params->niod.iod_nr += p, s_cur += len*k) {
		daos_recx_t *nrecx;

		rc = allocate_parity(&(params->p_segs), len, p,
				     params->niod.iod_nr);
		if (rc != 0)
			return rc;
		...
		for (i = 0; i < p; i++) {
			 params->niod.iod_recxs[params->niod.iod_nr+i].rx_idx =
			    PARITY_INDICATOR | 
			    ((s_cur+(unsigned long)(i*len))/params->niod.iod_size);
...
/* Indentifies the applicable subset of forwarding targets for non-full-stripe
 * EC updates.
 */
static void
