...
struct oid_scan_args {
	...
	struct oid_bucket	oa_buckets[OID_BUCKET_MAX];
};

static int
cont_send_oid_bucket(struct oid_scan_args *oa, uint32_t bucket_id)
{
	struct oid_bucket *bucket = &oa->oa_buckets[bucket_id];
	int		   i;
	int		   rc;
...
{
	struct oid_scan_args	*oa = data;
	struct oid_bucket	*bucket;
	daos_obj_id_t		 oid;
	unsigned int		 bid;
	int			 rc;
	...
	rc = cont_send_oid_bucket(oa, bid);
