...
static void extent_twist(struct extent *input, struct extent *output, int off,
			 bool add)
{
	*output = *input;
	if (add) {
...
static int update_array(int index, int eph, struct extent *extents,
			...
			char *output_buf)
{
	return update_array_internal(index, eph, extents, extents_num, rec_size,
...
static int punch_array(int index, int eph, struct extent *extents,
		       ...
		       char *output_buf)
{
	return update_array_internal(index, eph, extents, extents_num, 0,
...
int generate_io_conf_akey(int fd, struct current_status *status)
{
	char akey[64];
	int  rc = 0;
...
int generate_io_conf_dkey(int fd, struct current_status *status)
{
	char dkey[64];
	int  rc = 0;
...
int generate_io_conf_obj(int fd, struct current_status *status)
{
	char oid_buf[64];
	int  rc = 0;
...
int main(int argc, char **argv)
{
	...
	while ((rc = getopt_long(argc, argv, "a:d:o:s:g:t:O:h", long_ops,
				 NULL)) != -1) {
		char *endp;
